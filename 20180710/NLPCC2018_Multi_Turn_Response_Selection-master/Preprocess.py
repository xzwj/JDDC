...
def generate_test_pkl(data_path, candidate_path, raw_pkl_path, seg_pkl_path, word_dict, index_pkl_path, with_unk=True):
    file = open(data_path, encoding="utf-8", mode="r")
    file2 = open(candidate_path, encoding="utf-8", mode="r")
    conversations = []
    utterances = []
    candidates = []
    utt_temp = []
    for line in file.readlines():
        if line.strip():
            utt_temp.append(line.strip())
        else:
            utterances.append(utt_temp)
            utt_temp = []

    can_temp = []
    for line in file2.readlines():
        if line.strip():
            can_temp.append(line.strip())
        else:
            candidates.append(can_temp)
            can_temp = []
    assert len(utterances) == len(candidates)

    for utt,can in zip(utterances,candidates):
        context = utt
        true_response = can[0]
        false_response = can[1:]
        conversations.append(Conversation(context=context, true_response=true_response, false_response=false_response))

    pickle.dump(conversations, open(raw_pkl_path, 'wb+'), protocol=True)

    tokenizer = Tokenizer()
    test = pickle.load(open(raw_pkl_path, 'rb'))
    for con in test:
        con.word_segmentation(tokenizer)
    pickle.dump(test, open(seg_pkl_path, 'wb+'), protocol=True)

    test = pickle.load(open(seg_pkl_path, 'rb'))
    for con in test:
        con.word2index_with_dict(word_dict, with_unk=with_unk)
    pickle.dump(test, open(index_pkl_path, 'wb+'), protocol=True)

    test = pickle.load(open(index_pkl_path, 'rb'))
    contexts = []
    labels = []
    responses = []
    for con in test:
    ...
    pickle.dump([contexts, responses, labels], open('./data/Evaluate_test50.pkl', 'wb+'), protocol=True)
...
if __name__ == "__main__":
    ...
    # val = pickle.load(open('./data/test.raw.pkl', 'rb'))
    # for con in val:
    #     print(len(con.false_response))
    # print("all work has finished")

    word_embedding = load_word_embedding()
    generate_test_pkl('data/test50.txt', 'data/result50.txt', 'data/result50.raw.pkl', 'data/result50.seg.pkl', word_embedding.vocab_hash, 'data/result50.index.pkl', with_unk=False)
    # pass

    # val = pickle.load(open('./data/result50.raw.pkl', 'rb'))
    # for con in val:
    #     print(len(con.false_response))